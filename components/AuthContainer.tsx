...
const MockGoogleButton = ({ onClick, isLoading }: any) => (
  <button
    type="button"
    onClick={onClick}
    disabled={isLoading}
    className="w-full flex items-center justify-center gap-3 py-3.5 px-4 rounded-2xl border border-slate-200 dark:border-white/5 bg-white dark:bg-[#0a1815] hover:bg-slate-50 dark:hover:bg-slate-900/50 text-slate-800 dark:text-slate-200 text-xs font-bold transition-all active:scale-[0.98] cursor-pointer"
  >
    ...
    <span>ورود با حساب گوگل (شبیه‌ساز)</span>
  </button>
);
...
export const AuthContainer: React.FC<AuthContainerProps> = ({ brandColor, onBackToLanding, onSuccess, initialIntent = 'normal', onProceedAsCustomer }) => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    setIsLoading(true);
    try {
      // Simulated Google auth: creates a fully verified user instantly in repository
      const googleEmail = 'google.user@gmail.com';
      try {
        await authRepository.register(googleEmail, 'google-pass-123', 'حسین', 'رحمانی');
      } catch (_) {
        // If already registered, perform normal login
      }
      const session = await authRepository.loginWithEmail(googleEmail, 'google-pass-123');
      // Set status directly to ACTIVE since Google is pre-verified
      if (session.user) {
        await authRepository.verifyEmail(session.user.id, '123456');
      }
      await refetchSession();
    } catch (err: any) {
      setGlobalError(err.message || 'خطا در اتصال به گوگل');
    } finally {
      setIsLoading(false);
    }
  };
          ...
          <AnimatePresence mode="wait">
            ...
            {activeScreen === 'login' && (
              ...
              >
                ...
                <MockGoogleButton onClick={handleGoogleSignIn} isLoading={isLoading} />
